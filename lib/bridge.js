...
function warnIfNotSupportHybrid (message) {
    return {
        send () {
...
function initBridge () {
    ...
    if (window.bbhybrid && window.bbhybrid.bridge) {
        // 如果有@beibei/hybrid且版本大于1.0.5
        return window.bbhybrid.bridge
    }
    */
    if (window.bbhybrid) {
        return warnIfNotSupportHybrid('please upgrade @beibei/hybrid to version 1.0.5')
    }

    if (UA.indexOf('Hybrid') === -1) {
        return warnIfNotSupportHybrid('current env is not support hybrid')
    }
